...
const MyCourses = () => {
  ...
  useEffect(() => {
    if (user?.email) {
      axiosPublic.get(`/student-course/${user?.email}`).then((result) => {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(
            `https://course-pilot-serverr.vercel.app/student-course/${id}`
          )
          .then((response) => {
    ...
    <div className="p-6 bg-gray-50 min-h-screen rounded-xl shadow-md">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent>
                      <DropdownMenuItem className="cursor-pointer" asChild>
                        <Link
                          href={`/instructor-dashbord/my-courses/update-course/${course._id}`}
                        >
      ...
      <div className="flex items-center justify-between mt-4">
        <span className="text-sm text-gray-500">
          ...
          {Math.min(indexOfLastCourse, courses.length)} of {courses.length}{" "}
          entries
        </span>
